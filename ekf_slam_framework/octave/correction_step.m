...
for i = 1:m
  ...
  theta=mu(3);
    ...
    mu(2*landmarkId+2)=x+range*cos(theta+bearing);
    mu(2*landmarkId+3)=y+range*sin(theta+bearing);
  ...
  delta_x=mu(2*landmarkId+2)-x;
  delta_y=mu(2*landmarkId+3)-y;
  delta=[delta_x;delta_y];
  q=delta'*delta;
  
  expectedZ(2*i-1)=sqrt(q);
  expectedZ(2*i)=normalize_angle(atan2(delta_y,delta_x)-theta)

	% TODO: Compute the Jacobian Hi of the measurement function h for this observation
	Hit=(1/q)*[-sqrt(q)*delta_x,-sqrt(q)*delta_y,0,sqrt(q)*delta_x,sqrt(q)*delta_y;delta_y,-delta_x,-q,-delta_y,delta_x];
  Fxj=zeros(5,length(mu));
  Fxj(1:3,1:3)=eye(3)
