...
distance_matrix=sqrtm(sigma*(n+lambda));
% TODO: compute all sigma points
%we have 2n+1 term for each mu 
sigma_points(:,1)=mu;
for i=1:n
  sigma_points(:,i+1)=mu+distance_matrix(:,i);
endfor

for i=n+1:2*n
  sigma_points(:,i+1)=mu+distance_matrix(:,i-n);
endfor
...
w_m0=lambda/(n+lambda);
w_c0=w_m0+(1-alpha^2+beta);
w=(1/2/(n+lambda))*ones(1,2*n);
